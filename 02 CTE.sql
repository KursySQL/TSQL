...
;WITH cte AS 
(
	SELECT TOP 3 ps.ProductSubcategoryID, ps.Name AS SubcategoryName, COUNT(*) AS Cnt
	FROM Production.Product AS p
	JOIN Production.ProductSubcategory AS ps ON ps.ProductSubcategoryID = p.ProductSubcategoryID
	GROUP BY ps.ProductSubcategoryID, ps.Name
	ORDER BY COUNT(*) DESC
)
SELECT DISTINCT soh.SalesOrderID, soh.OrderDate, soh.SubTotal, cte.SubcategoryName 
FROM cte
...
(
	SELECT DISTINCT 
		soh.SalesOrderID, soh.OrderDate, soh.SubTotal, cte_products.SubcategoryName, soh.CustomerID 
	FROM cte_products
...
LEFT JOIN HumanResources.EmployeeOldSchool AS emp ON emp.EmployeeID = manager.ManagerID
